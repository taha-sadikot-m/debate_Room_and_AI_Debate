...
import { ArrowLeft, Users, Clock, Trophy, Flag, MessageSquare, Zap, Mic, MicOff, Volume2, Send, Bot, Timer, Star, Copy, UserPlus, LogIn, Share2, Link, CheckCircle } from 'lucide-react';
...
const LiveDebateRoomV3 = ({ 
  ...
  const [currentRoomId, setRoomId] = useState<string>(roomId || '');
  const [roomStatus, setRoomStatus] = useState<'waiting' | 'connected' | 'error'>(
    roomId ? 'waiting' : 'connected'
  );
  ...
  const [endDebateRequested, setEndDebateRequested] = useState<'none' | 'requested' | 'pending' | 'approved'>('none');
    ...
    const checkInterval = setInterval(() => {
      ...
      if (opponentData) {
        try {
          ...
          // Initialize the debate with the joined opponent
          initializeDebate();
        } catch (e) {
  ...
  const handleJoinRoom = () => {
    ...
    try {
      ...
      const currentUser: OnlineUser = {
        ...
        country: 'Global',
        status: 'available',
        side: 'FOR'  // Default value, will be set correctly based on host's side
      };
      ...
      toast({
        ...
        description: `You've joined ${hostUser.name}'s debate room`,
        variant: "default"
      });
            ...
            <DialogFooter className="flex flex-col gap-2 sm:flex-row sm:justify-between">
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  // Store as host in localStorage
                  localStorage.setItem(`debate_room_${currentRoomId}_host`, JSON.stringify(hostUser));
                  ...
                  const checkInterval = setInterval(() => {
                    const opponentData = localStorage.getItem(`debate_room_${currentRoomId}_opponent`);
                    if (opponentData) {
                      try {
                        const opponent = JSON.parse(opponentData);
                        setRealOpponent(opponent);
                        ...
                        toast({
                          title: "Opponent Joined!",
                          description: `${opponent.name} has joined your debate room`,
                        });
                      } catch (e) {
                        console.error("Failed to parse opponent data", e);
                      }
                  ...
                  setTimeout(() => clearInterval(checkInterval), 300000); // 5 minute timeout
                }}
              >
                Create Room
              </Button>
  ...
  const handleStartDebate = () => {
    if (currentRoomId && roomStatus === 'waiting') {
      toast({
  ...
  const initializeDebate = () => {
    // In a real debate, the host always speaks first
    const speakFirst = isRoomHost;
    
    // Set who speaks first based on host status
    setIsMyTurn(speakFirst);
    setCurrentSpeaker(speakFirst ? 'You' : realOpponent ? realOpponent.name : 'Opponent');
    
    // Determine and display sides
    const mySide = isRoomHost ? 'FOR' : 'AGAINST';
    setAssignedSide(mySide);
    
    toast({
      title: "Debate Started!",
      description: `You are arguing ${mySide}. ${speakFirst ? 'You speak first' : 'Your opponent speaks first'}.`,
    });
    ...
    const initialMessage: LiveDebateMessage = {
      ...
      message: `The debate has begun! The topic is: ${topic?.title || 'Unknown Topic'}. ${speakFirst ? 'You' : 'Your opponent'} speaks first.`,
      timestamp: new Date(),
      isUser: false
    };
  ...
  const endDebate = () => {
    ...
    if (realOpponent && endDebateRequested === 'none') {
      ...
      localStorage.setItem(`debate_room_${currentRoomId}_end_requested`, JSON.stringify({
        requestedBy: 'host', // or 'opponent' based on isRoomHost
      ...
      setTimeout(() => {
        clearInterval(checkInterval);
        if (endDebateRequested === 'requested') {
      ...
      localStorage.setItem(`debate_room_${currentRoomId}_end_approved`, JSON.stringify({
        approvedBy: 'host', // or 'opponent' based on isRoomHost
        timestamp: new Date().toISOString()
      }));
